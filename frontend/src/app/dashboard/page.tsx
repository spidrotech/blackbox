...
const PROJECT_BADGE: Record<string, { label: string; cls: string; dot: string }> = {
  draft:       { label: 'Brouillon',  cls: 'bg-slate-50 text-slate-600 ring-1 ring-slate-200',     dot: 'bg-slate-400' },
              ...
              {d.recentProjects.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-50">
                  {d.recentProjects.slice(0, 6).map(p => {
                    const pb = p.type !== 'quote_worksite' ? (PROJECT_BADGE[p.status] ?? PROJECT_BADGE.draft) : null;
                    return (
                    <li key={`${p.type ?? 'project'}-${p.id}`} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                      <div className="min-w-0">
                        {p.type === 'quote_worksite' ? (
                          <Link href={buildDetailPath('quotes', p.id)} className="text-sm font-medium text-gray-800 hover:text-violet-600 truncate block">{p.name}</Link>
                        ) : (
                          <Link href={buildDetailPath('projects', p.id)} className="text-sm font-medium text-gray-800 hover:text-violet-600 truncate block">{p.name}</Link>
                        )}
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {p.worksite_address && (
                            <span className="text-xs text-violet-500 flex items-center gap-0.5">
                              ...
                              {p.worksite_address.slice(0, 25)}{p.worksite_address.length > 25 ? '…' : ''}
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="text-right shrink-0 ml-4 flex items-center gap-2">
                        {p.type === 'quote_worksite' ? (
                          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-violet-50 text-violet-600 ring-1 ring-violet-200">Devis</span>
                        ) : (
                          <>
                            {pb && (
                              <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${pb.cls}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${pb.dot}`} />
                                {pb.label}
                              </span>
                            )}
                            {p.budget ? <span className="text-sm font-semibold text-gray-700">{fmt(p.budget)}</span> : null}
                          </>
                        )}
                      </div>
                    </li>

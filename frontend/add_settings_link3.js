const fs = require('fs');
const path = 'C:/workspace/blackbox/frontend/src/app/quotes/[id]/page.tsx';

let content = fs.readFileSync(path, 'utf8');

const target = '<button onClick={handleDownloadPdf} className="text-xs text-blue-600 hover:underline">TÃ©lÃ©charger</button>';
...
if (content.includes(target)) {
  ...
  console.log("Updated [id]/page.tsx");
} else {
    // maybe encoded differently
    const t2 = '<button onClick={handleDownloadPdf} className="text-xs text-blue-600 hover:underline">Télécharger</button>';
    if (content.includes(t2)) {
        content = content.replace(t2, replacement);
        fs.writeFileSync(path, content, 'utf8');
        console.log("Updated [id]/page.tsx (t2)");
    } else {
